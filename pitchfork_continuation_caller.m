...
cont_obj_pre=NSTransH_VDconst_contRi_G0_sym(mesh,settings,0.2,1000,1);
cont_obj_pre=cont_obj_pre.run_cont_forward();
settings.Gamma=cont_obj_pre.para_out(1,end);
settings.G0=0;
cont_obj=NSTransH_VDconst_contRi_G0_sym(mesh,settings,0.2,1000,1,cont_obj_pre.x_out(:,end));
...
Nx=exp(cont_obj.x_out(N_mesh+2:(2*N_mesh+1),:));
...
Uxf=figure('Position',[100,320,300,320]);hold on;
colororder(Uxf,col_order);Uxa=gca;
set(Uxa,'FontSize',14);
Nxf=figure('Position',[400,320,300,320]);hold on;
...
plot(Bifur_a,cont_obj.para_out * settings.Re/settings.Pe_s,Ux(N_mesh,:) /settings.Pe_s,'b-');

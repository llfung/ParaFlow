...
cont_obj_pre=NSTransH_VDconst_contRi_G0_radial(mesh,settings,0.2,1000,1);
...
Urf=figure('Position',[100,320,300,320]);hold on;
colororder(Urf,col_order);Ura=gca;
set(Ura,'FontSize',14);
Nrf=figure('Position',[400,320,300,320]);hold on;
...
plot(Bifur_a,cont_obj.para_out(1:ind0-1) * settings.Re/settings.Pe_s,Ur(N_mesh,1:ind0-1) /settings.Pe_s,'b-');
plot(Bifur_a,[8 8],[-20 20],'r:');
plot(Bifur_a,cont_obj.para_out(ind0:end) * settings.Re/settings.Pe_s,Ur(N_mesh,ind0:end) /settings.Pe_s,'b--');
%plot(Bifur_a,pi^2+[-10:0.01:10].^2*pi^2/12,[-10:0.01:10],'r:');
plot(Bifur_a,[0 14.68197064212389],[0 0],'b-');
plot(Bifur_a,[14.68197064212389 100],[0 0],'b--');

for i=1:length(U0_select)
    ind=find(Ur(N_mesh,:)<U0_select(i),1,'first');
    plot(cont_obj.para_out(ind),Ur(N_mesh,ind),'o','Color',col_order(i,:));
end
hold off;

legend('Amplitude $$u(0)$$','$$Ri Re \xi n_0/Pe_s =8$$','location','southeast','interpreter','latex','FontSize',12);

classdef NSTransH_VDconst_contRi_radial < ode_continuation
    ...
    methods
        %% Constructor
        function obj=NSTransH_VDconst_contRi_radial(mesh_obj,settings,h,num_iter,save_iter,x0)
            if nargin<4
                error('NSTransH_VDconst_contRi_radial: Not enough Input argument');
            else
        ...
        function [res,lhs]=residue_infun(obj,y,yp,lib_value,z)
            ...
            Nbar=ones(N,1);
            lhs2=-G+1/obj.Re*(rinv.*DU0+D2U0)-Gamma*(exp(H0)-Nbar);
            lhs2(end)=0;
            lhs2(1)=0;
        ...
        function dfdx=dfdx_infun(obj,y,lib_value,z)
            ...
            % Preparing Jacobian 
            % (Flow Rate)
            dfdx1 = [0  2*wint zeros(1,N+1)];

            % (Navier-Stokes)
            dfdx2 = [-ones(obj.mesh_obj.N,1)  1/obj.Re*(diag(rinv)*D1M+D2M)  -Gamma*diag(exp(H0)) -(exp(H0)-1)];
        ...
        function x0=get_initial_solution(obj)
            ...
            prof_obj=NSTransH_VDconst_radial(obj.mesh_obj,settings);

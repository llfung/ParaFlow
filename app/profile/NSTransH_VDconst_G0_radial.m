classdef NSTransH_VDconst_G0_radial < solv_prof
    ...
    methods
        %% Constructor
        function obj=NSTransH_VDconst_G0_radial(mesh_obj,settings,ini_prof)
            switch nargin
                case 2
        ...
        function [res,lhs]=residue_infun(obj,x,lib_value)
            ...
            Nbar=ones(N,1);
            lhs2=-obj.G0+1/obj.Re*(rinv.*DU0+D2U0)-obj.Gamma*(exp(H0)-Nbar);
            lhs2(end)=0;
            lhs2(1)=0;
        ...
        function dfdx=dfdx_infun(obj,y,lib_value)
            ...
            % Preparing Jacobian
            % (Flow Rate)
            dfdx1=[-1  2*wint zeros(1,N)];

            % (Navier-Stokes)
            dfdx2=[zeros(obj.mesh_obj.N,1) 1/obj.Re*(diag(rinv)*D1M+D2M)   -obj.Gamma*diag(exp(H0))];
        ...
        function obj=get_ini(obj,x0)
            if nargin>1
                ...
                Q  = 2*obj.mesh_obj.wint*(obj.mesh_obj.col_pt.*U0);
